...
// ============================================
// COMPOSANT PROTÉGÉ — redirige vers login si
...
function App() {
  ...
  useEffect(() => {
    // Vérifie la session dans le localStorage
    const session = localStorage.getItem('parkinfo_session')
    if (session === 'connecte') {
      setConnecte(true)
    }
    setChargement(false)
  ...
  // Affiche un écran de chargement le temps de vérifier la session
  if (chargement) {
    return (
        ...
        <Route
          ...
          element={
            connecte
              ...
              : <Login onLogin={() => setConnecte(true)} />
          }
        />
